...
class FormHelper
{
	...
	{
		$tmp = array();
		//Se essa tag puder incluir um atributo de valor, tiver um nome e houver uma entrada para o nome no array values, define um atributo de valor
		...
		foreach($attributes as $k => $v)
		{
			...
			if(is_bool($v))
			{
				if ($v) {$tpm[] = $this->encode($k);}
			}
			//caso contrário, k=v
			else
			{
				$value = $this->encode($v);
				//Se esse elemento puder ter mais de um valor acrescent [] ao seu nome
	...
	protected function options($name, $options)
	{
		...
		{
			$s = "<option value=\"{$this->encode}\"";
			if($this->isOptionSelected($name, $k))
			{
				$s .= 'selected';
			}
			$s .= ">{$this->encode($v)}</otpion>";
			$tmp[] = $s;
			}
			return implode('', $tmp);
		}
		...
		{
			//Se não houver uma entrada para $name no array values, essa opção não poderá ser selecionada
